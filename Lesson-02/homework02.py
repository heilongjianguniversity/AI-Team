...
def normalization(X):
    Xmin = #请补全                                         # 请补全 通过使用 np.min 函数，计算原始数据沿着 axis=0 方向的最小值，即：求每一列的最小值，并赋值给 Xmin
    Xmax = #请补全                                         # 请补全 通过使用 np.max 函数，计算原始数据沿着 axis=0 方向的最大值，即：求每一列的最大值，并赋值给 Xmax
    Xmu  = #请补全                                         # 请补全 通过使用 np.mean 函数，计算原始数据均值，并赋值给 Xmu                 
    X_norm =  #请补全                                      # 请补全 计算归一化后的数据，归一化公式为：(X-Xmu)/(Xmax-Xmin)，归一化后数据范围为 [-1,1]
